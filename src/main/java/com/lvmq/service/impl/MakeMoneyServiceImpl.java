...
public class MakeMoneyServiceImpl implements MakeMoneyService {
	...
	public List<MakeMoneyTaskRes> makeMoneyTask(String userId,String page,String pageSize){
		...
		for(MakeMoneyLog m:makeMoneyLogList) {
			...
			mlr.add(new MakeMoneyTaskRes(m.getId(),ml.get().getLogo(),ml.get().getTitle(),simpleDateFormat.format(m.getEndTime()),status,rewards,ml.get().getId(),
					String.valueOf(m.getEndTime().getTime()-new Date().getTime())));
		}
		return mlr;
	}
